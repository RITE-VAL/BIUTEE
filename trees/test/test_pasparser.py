...
class TestPASParser(unittest.TestCase):
    ...
    def test_pas1_tree(self):
        tree1 = self.ins1[0]
        self.assertEqual(len(tree1), 5)
        for n in tree1:
            self.assertTrue(isinstance(n, node.Node))

    def test_pas1_node(self):
        tree1 = self.ins1[0]
        txt = ""
        for n, c in zip(tree1, [2, 2, 2, 2, 1]):
            self.assertEqual(len(n), c)
            txt += n.get_surface()
    ...
    def test_pas2_node(self):
        ...
        tree2 = self.ins2[1]
        txt = ""
        for n, c in zip(tree1, [2, 3, 3, 1]):
            self.assertEqual(len(n), c)
            txt += n.get_surface()
        self.assertEqual(u"花子は果物屋に行った．", txt)
        txt = ""
        for n, c in zip(tree2, [1, 2, 3, 1]):
            self.assertEqual(len(n), c)
            txt += n.get_surface()
